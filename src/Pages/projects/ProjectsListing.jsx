...
const ProjectsListing = () => {
  ...
  const selectClass = "bg-white/10 border border-white/20 rounded-xl px-3 py-2.5 min-h-[44px] text-white text-sm focus:border-orange-400 focus:outline-none transition-all appearance-none";
        ...
        <main className="pb-16 sm:pb-20">
          <div className="container mx-auto px-4 sm:px-6 py-6 sm:py-8 max-w-6xl">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
              <div>
                <h1 className="text-2xl sm:text-3xl font-black text-white" style={{ fontFamily: '"Inter", sans-serif' }}>
                  <span className="text-transparent bg-clip-text bg-gradient-to-r from-orange-400 to-orange-500">Projects</span>
                </h1>
              ...
              <Link to="/projects/submit"
                className="inline-flex items-center justify-center px-5 py-2.5 min-h-[44px] bg-gradient-to-r from-orange-500 to-orange-600 hover:from-orange-600 hover:to-orange-700 text-white font-bold rounded-xl text-sm transition-all shadow-lg">
                Post a Project
              </Link>
            </div>

            {/* Search + Filters */}
            <div className="bg-white/5 border border-white/20 rounded-2xl p-4 mb-6 space-y-3">
              <input
                type="text" value={searchQuery} onChange={e => setSearchQuery(e.target.value)}
                className="w-full bg-white/10 border border-white/20 rounded-xl px-4 py-3 min-h-[44px] text-white placeholder-gray-400 focus:border-orange-400 focus:outline-none text-sm transition-all"
              ...
              <div className="flex flex-wrap gap-2">
                ...
                {(searchQuery || filters.industryTrack || filters.timeline || filters.pricingType) && (
                  <button onClick={clearFilters} className="text-orange-400 hover:text-orange-300 text-xs font-semibold px-3 py-2 transition-colors">Clear All</button>
                )}
              </div>
            </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {filteredProjects.map(project => (
                  <div key={project.id}
                    ...
                    className="bg-gradient-to-br from-black/40 via-gray-900/40 to-black/40 rounded-xl p-4 sm:p-5 border border-white/20 shadow-xl cursor-pointer hover:border-orange-400/40 hover:bg-white/5 transition-all duration-200 active:scale-[0.99]"
                  >
                    <div className="flex items-start justify-between gap-3 mb-3">
                      <h3 className="text-white font-bold text-sm sm:text-base line-clamp-2">{project.projectTitle}</h3>
                      <span className={`flex-shrink-0 px-2 py-0.5 rounded-full text-[10px] font-bold ${project.pricingType === 'paid' ? 'bg-orange-500/20 text-orange-300 border border-orange-500/30' : 'bg-green-500/20 text-green-300 border border-green-500/30'}`}>
                    ...
                    <div className="flex flex-wrap gap-1.5 mb-3">
                      ...
                      <span className="px-2 py-0.5 bg-white/10 rounded-md text-gray-300 text-[10px] font-medium">{project.maxTeamSize || 0} team members</span>
                    </div>
