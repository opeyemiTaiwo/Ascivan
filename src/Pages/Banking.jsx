// src/Pages/Banking.jsx - Banking & Financial Services for International Students
...
const Banking = () => {
  ...
  const serviceTypes = [
    ...
    { id: 'tax-services', label: 'Tax Services' },
    { id: 'financial-aid', label: 'Financial Aid' },
  ];

  useEffect(() => {
    if (!authLoading && !currentUser) {
      navigate('/login', { replace: true, state: { from: '/banking' } });
    }
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(q, (snapshot) => {
      ...
      console.error('Error fetching banking posts:', error);
      setLoading(false);
    });
  ...
  const getTypeBadge = (type) => {
    ...
    return map[type] || { label: type || 'Financial Service', cls: 'bg-orange-500/20 text-orange-300' };
  };
      ...
      <>
        ...
        <div className="min-h-screen flex items-center justify-center" style={{ backgroundColor: '#000' }}>
          <div className="text-center">
            ...
            <p className="text-white">Loading Banking Services...</p>
          </div>
        </div>
      </>
    );
            ...
            {filteredPosts.length === 0 ? (
              <div className="text-center py-16">
                ...
                <h3 className="text-2xl font-bold text-white mb-3">
                  {posts.length === 0 ? 'No services listed yet' : 'No services match your search'}
                </h3>
                <p className="text-gray-400 mb-8">
                  {posts.length === 0 ? 'Be the first to list a financial service!' : 'Try adjusting your filters.'}
                </p>
                <button onClick={() => navigate('/banking/post')} className="px-6 py-3 bg-gradient-to-r from-orange-500 to-orange-600 text-white font-bold rounded-xl transition-all">
                  List a Service
                </button>
              </div>
            ) : (
                  ...
                  return (
                    <div key={post.id} className="group">
                      <div className={`bg-white/5 backdrop-blur-sm border rounded-xl p-5 sm:p-6 hover:shadow-lg hover:shadow-orange-500/10 transition-all duration-300 h-full flex flex-col ${isClosed ? 'border-white/10 opacity-75' : 'border-white/20 hover:border-orange-500/30'}`}>
                        ...
                        {post.providerName && <p className="text-orange-400 text-sm font-semibold mb-2">{post.providerName}</p>}
                        ...
                        <div className="mt-auto pt-4 border-t border-white/10">
                          <div className="flex justify-between text-xs text-gray-400 mb-3">
                            <span className="truncate max-w-[140px]">{post.posterName}</span>
