  ...
  return (
    <div className="relative">
      ...
      <button
        ...
        className="relative text-white hover:text-lime-400 transition-all duration-300 group p-1 sm:p-0"
      ...
      {showDropdown && (
        <div className="fixed xs:absolute left-2 right-2 xs:left-auto xs:right-0 top-16 xs:top-full mt-0 xs:mt-2 w-auto xs:w-80 sm:w-96 md:w-[26rem] lg:w-[28rem] bg-gray-900/98 xs:bg-gray-900/95 backdrop-blur-xl border border-white/20 rounded-xl xs:rounded-2xl shadow-2xl z-[9999] max-h-[calc(100vh-5rem)] xs:max-h-[85vh] sm:max-h-96 overflow-hidden">
            ...
            <div className="flex items-center gap-1.5 xs:gap-2">
              {unreadCount > 0 && (
                <button
                  ...
                  className="text-lime-400 hover:text-lime-300 text-[11px] xs:text-xs font-medium transition-colors px-2 py-1 hover:bg-white/5 rounded hidden xs:block"
                >
                  Mark all read
                </button>
              )}
          ...
          {unreadCount > 0 && (
            <div className="xs:hidden px-3 py-2 border-b border-white/10 bg-gray-900/50">
              <button
                onClick={markAllAsRead}
                className="text-lime-400 hover:text-lime-300 text-xs font-medium transition-colors w-full text-left"
              >
                ✓ Mark all as read
              </button>
            </div>
          ...
          <div className="overflow-y-auto max-h-[calc(100vh-12rem)] xs:max-h-60 sm:max-h-80 overscroll-contain">
            {loading ? (
              <div className="p-6 xs:p-8 sm:p-10 text-center">
                <div className="animate-spin rounded-full h-6 w-6 xs:h-8 xs:w-8 border-b-2 border-lime-400 mx-auto mb-3"></div>
              ...
              <div className="divide-y divide-white/5">
                {notifications.slice(0, 20).map((notification) => (
                  <div
                    ...
                    className={`p-3 xs:p-3.5 sm:p-4 hover:bg-white/5 active:bg-white/10 cursor-pointer transition-all duration-200 group ${
                      !notification.isRead ? 'bg-lime-400/5 border-l-2 xs:border-l-[3px] border-lime-400' : ''
                  ...
                  >
                    <div className="flex items-start gap-2 xs:gap-2.5 sm:gap-3">
                      ...
                      <div className="flex-shrink-0 w-7 h-7 xs:w-8 xs:h-8 sm:w-9 sm:h-9 bg-lime-500/20 rounded-full flex items-center justify-center group-hover:bg-lime-500/30 transition-colors">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="flex items-center gap-1.5 xs:gap-2 mb-1">
                          {notification.mentionedByPhoto ? (
                            ...
                            <div className="w-4 h-4 xs:w-5 xs:h-5 bg-gray-600 rounded-full flex items-center justify-center flex-shrink-0">
                              <span className="text-[10px] xs:text-xs text-white font-bold">
                                {(notification.mentionedByFirstName || notification.mentionedByName || 'U').charAt(0).toUpperCase()}
                              </span>
                            </div>
                          )}
                          <span className="text-white font-medium text-xs xs:text-sm truncate">
                          ...
                          {!notification.isRead && (
                            <div className="w-1.5 h-1.5 xs:w-2 xs:h-2 bg-lime-400 rounded-full flex-shrink-0 animate-pulse"></div>
                          )}
                        </div>

                        {/* Action text - Responsive */}
                        <p className="text-gray-300 text-xs xs:text-sm mb-1.5 xs:mb-2 line-clamp-2">
                        ...
                        {(notification.replyContent || notification.repostComment) && (
                          <p className="text-gray-400 text-[11px] xs:text-xs bg-black/20 rounded px-2 py-1.5 xs:p-2 mb-1.5 xs:mb-2 line-clamp-2">
                            "{notification.replyContent || notification.repostComment}"
                          </p>
                        )}
                        ...
                        <div className="flex items-center justify-between gap-2">
                          ...
                          <div className="flex items-center gap-1.5 xs:gap-2 min-w-0">
                            {notification.postTitle && (
                              <span className="text-lime-400 text-[11px] xs:text-xs truncate max-w-[60px] xs:max-w-20 sm:max-w-24">
                                "{notification.postTitle}"
                              </span>
                            )}
                            ...
                            {navigating === notification.id ? (
                              <div className="w-3 h-3 xs:w-3.5 xs:h-3.5 border border-lime-400 border-t-transparent rounded-full animate-spin flex-shrink-0"></div>
                            ) : (
                              <div className="text-gray-400 group-hover:text-lime-400 transition-colors opacity-0 group-hover:opacity-100 flex items-center text-[11px] xs:text-xs flex-shrink-0">
          ...
          {notifications.length > 0 && (
            <div className="p-3 xs:p-3 sm:p-4 border-t border-white/10 bg-black/20 sticky bottom-0">
              <button
                onClick={handleViewAllNotifications}
                className="w-full text-center text-lime-400 hover:text-lime-300 text-xs xs:text-sm font-medium transition-colors py-1 hover:bg-white/5 rounded"
              >
      ...
      {showDropdown && (
        <div 
          className="fixed inset-0 z-[9998] bg-black/20 xs:bg-transparent" 
          onClick={() => setShowDropdown(false)}
        />
      )}
    </div>
